...
function makeWorkingStorage(namespace) {
  return new Storage({ namespace, force_local_storage: true });
}
...
describe('helpers/Persister', function () {
  ...
  beforeEach(function () {
    ...
    persister.storage = makeWorkingStorage(persister.options.namespace);
  });
  ...
  it('stores and retrieves data by state key', function () {
    ...
    const result = persister.get('state1');
  ...
  it('different state keys are stored independently', function () {
    ...
    expect(persister.get('stateA').token).toBe('a');
    expect(persister.get('stateB').token).toBe('b');
  });
});
